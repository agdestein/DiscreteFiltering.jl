"""
    solve(
        equation::BurgersEquation{PeriodicIntervalDomain{T},F},
        u,
        tlist,
        M,
        N;
        method = "filterfirst",
        solver = QNDF(),
        reltol = 1e-4,
        abstol = 1e-6,
    ) where {T,F}

Solve `equation` from `tlist[1]` to `tlist[2]` with initial conditions `u` and a
discretization of `M` and `N` points. If `method` is `"filterfirst"`, the equation is filtered then
discretized. If `method` is `"discretizefirst"`, the equation is discretized then filtered.
"""
function solve(
    equation::BurgersEquation{PeriodicIntervalDomain{T},F},
    u,
    tlist,
    M,
    N;
    method = "filterfirst",
    subspacedim = 10,
    solver = QNDF(),
    reltol = 1e-4,
    abstol = 1e-6,
    degmax = 10,
    λ = 1e-6,
    λ_ridge = 1e-8,
) where {T,F}
    @unpack domain, filter, ν = equation

    x = discretize(domain, M)
    ξ = discretize(domain, N)

    ū = apply_filter(u, filter, domain)

    h = filter.width

    # Get matrices
    C = advection_matrix(domain, N)
    D = diffusion_matrix(domain, N)

    if method == "filterfirst"
        error("Method \"filterfirst\" is not implemented for BurgersEquation")
    elseif method == "discretizefirst"
        # W = filter_matrix(filter, domain, M, N; degmax, λ)
        # R = reconstruction_matrix(filter, domain, M, N; degmax, λ)
        W, R = get_W_R(filter, domain, M, N; degmax, λ)
        ūₕ = ū.(x)
        # ūₕ = W * u.(ξ)
        p = (; J = -W * C * R, u² = copy(u))
        function Mdu!(du, u, p, t)
            k
            mul!(du, p.J, u)
        end
        odefunction = ODEFunction(
            Mdu!,
            jac = (J, u, p, t) -> (J .= p.J),
            jac_prototype = p.J,
            # mass_matrix = W * R,
        )
        problem = ODEProblem(odefunction, ūₕ, tlist, p)
        solution = OrdinaryDiffEq.solve(problem, solver; reltol, abstol)
    elseif method == "discretizefirst-without-R"
        W = filter_matrix(filter, domain, M, N; degmax, λ = λ_ridge)
        # println(W[M ÷ 2, :])
        # A = factorize(W'W + λ * I)
        A = lu(W'W + λ * I)
        J = -W * C
        ūₕ = ū.(x)
        # ūₕ = W * u.(ξ)
        p = (; u☆ = zeros(N), utmp = zeros(N), J, A)
        function Mdu!_without_R(dū, ū, p, t)
            @unpack u☆, utmp, J, A = p
            mul!(utmp, W', ū)
            ldiv!(u☆, A, utmp)
            mul!(dū, J, u☆)
        end
        odefunction = ODEFunction(
            Mdu!_without_R,
            # jac = (J, u, p, t) -> (J .= p.J),
            # jac_prototype = p.J,
            # mass_matrix = W * R,
        )
        problem = ODEProblem(odefunction, ūₕ, tlist, p)
        solution = OrdinaryDiffEq.solve(problem, solver; reltol, abstol)
    else
        error("Unknown method")
    end

    solution
end

function solve(
    equation::BurgersEquation{PeriodicIntervalDomain{T},IdentityFilter},
    u,
    tlist,
    M,
    N;
    solver = RK4(),
    reltol = 1e-4,
    abstol = 1e-6,
) where {T,F}
    @unpack domain, filter, ν = equation
    M == N || error("Only implemented for M = N")
    ξ = discretize(domain, N)

    # Get matrices
    C = advection_matrix(domain, N)
    D = diffusion_matrix(domain, N)

    uₕ = u.(ξ)
    p = (; u² = copy(uₕ), Cu² = copy(uₕ), Du = copy(uₕ))
    function du!(du, u, p, t)
        @unpack u², Cu², Du = p
        u² .= u.^2 
        mul!(Cu², C, u²)
        mul!(Du, D, u)
        @. du = -Cu² ./ 2 + ν * Du
        # du .= .-(C * u.^2) ./ 2 .+ ν .* (D * u)
    end
    odefunction = ODEFunction(du!)
    problem = ODEProblem(odefunction, uₕ, tlist, p)
    solution = OrdinaryDiffEq.solve(problem, solver; reltol, abstol)

    solution
end
